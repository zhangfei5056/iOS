...
#define kHeaderViewBottomMargin   10    //headerView下边界值
#define kFooterViewTopMargin      10    //footerView上边界值
...
@property (nonatomic, retain) NSLock* resetLock;
...
@property (nonatomic, retain) UIView             *headerIndicatorView;
@property (nonatomic, retain) UIView             *footerIndicatorView;
...
- (void)setup {
    ...
    self.resetLock = [[NSLock new] autorelease];
...
//	}
    ...
    if ((self.dragDir&DragDirectionHorizontal)!=0) {
        contentWidth = MAX(CGRectGetWidth(self.bounds)+1, contentWidth);
    }
    if ((self.dragDir&DragDirectionVertical)!=0) {
        contentHeight = MAX(CGRectGetHeight(self.bounds)+1, contentHeight);
    }
...
- (void)setHeaderIndicatorView:(UIView *)headerIndicatorView {
    if (headerIndicatorView_ != headerIndicatorView) {
        ...
        [headerIndicatorView_ release];
        
        headerIndicatorView_ = [headerIndicatorView retain];
...
- (void)setFooterIndicatorView:(UIView *)footerIndicatorView {
    if (footerIndicatorView_ != footerIndicatorView) {
        ...
        [footerIndicatorView_ release];
        
        footerIndicatorView_ = [footerIndicatorView retain];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (self.headerIndicatorViewEnabled &&
        self.headerIndicatorView &&
        [self.headerIndicatorView respondsToSelector:@selector(refreshScrollViewDidEndDragging:)]) {
        [self.headerIndicatorView performSelector:@selector(refreshScrollViewDidEndDragging:) withObject:scrollView];
    }
    
    if (self.footerIndicatorViewEnabled &&
        self.footerIndicatorView &&
        [self.footerIndicatorView respondsToSelector:@selector(refreshScrollViewDidEndDragging:)]) {
        [self.footerIndicatorView performSelector:@selector(refreshScrollViewDidEndDragging:) withObject:scrollView];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.headerIndicatorViewEnabled &&
        self.headerIndicatorView  &&
        [self.headerIndicatorView respondsToSelector:@selector(refreshScrollViewDidScroll:)]) {
        [self.headerIndicatorView performSelector:@selector(refreshScrollViewDidScroll:) withObject:scrollView];
    }
    
    if (self.footerIndicatorViewEnabled &&
        self.footerIndicatorView &&
        [self.footerIndicatorView respondsToSelector:@selector(refreshScrollViewDidScroll:)]) {
        [self.footerIndicatorView performSelector:@selector(refreshScrollViewDidScroll:) withObject:scrollView];
    }
