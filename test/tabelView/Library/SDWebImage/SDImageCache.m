...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        cacheInQueue.maxConcurrentOperationCount = 3;
        cacheOutQueue = [[NSOperationQueue alloc] init];
        cacheOutQueue.maxConcurrentOperationCount = 3;
...
- (BOOL)isImageCachedOnDisk:(NSString *)key
{
    NSFileManager * fm = [NSFileManager defaultManager];
    if ([fm fileExistsAtPath:[self cachePathForKey:key] isDirectory:NO]) {
        return YES;
    }
    return NO;
}
